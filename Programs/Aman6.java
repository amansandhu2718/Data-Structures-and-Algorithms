public class Aman6 {
    public static void main(String[] args) {
        ...
        root.children[1].children[1].children= new Node[]{ getNewNode(110),getNewNode(110)};

        
        TraverseGenericTreePreOrder(root);
    ...
    public static void TraverseGenericTreePreOrder(Node node){

        System.out.println(""+node.data);
        if(node!=null&& node.children!=null && node.children.length>0){
            for (int i = 0; i < node.children.length; i++) {
                TraverseGenericTreePreOrder(node.children[i]);
            }
        }
    }
