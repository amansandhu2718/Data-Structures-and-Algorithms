...
public class Aman236 {
    public static void main(String[] args) {
        ...
        System.out.println("smallest path is  : " + SmallestPath);
        PrintAllPathsWRTWeights(graph, 0, 6, visited, "0", 0);
        System.out.println("smallest path is wrt Weight  : " + SmallestPath);
    }

    public static String SmallestPath = null;
    public static int SmallestWeight = Integer.MAX_VALUE;

    public static void PrintAllPaths(ArrayList<Edge>[] graph, int src, int dest, boolean[] visited, String psf) {
        if (src == dest) {
            System.out.println(psf);
            if (SmallestPath == null || psf.length() < SmallestPath.length()) {
                SmallestPath = psf;
            }
            return;
        }
